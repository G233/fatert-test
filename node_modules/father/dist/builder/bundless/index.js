    ...
    try {
        ...
        for (let item of files) {
            ...
            if (config) {
                ...
                if (!opts.quiet) {
                    utils_1.logger.event(`Bundless ${utils_1.chalk.gray(item)} to ${utils_1.chalk.gray(itemDistPath)}${(result === null || result === void 0 ? void 0 : result.options.declaration) ? ' (with declaration)' : ''}`);
                }
                count += 1;
            }
            else {
        ...
        if (declarationFileMap.size) {
            utils_1.logger.event(`Generate declaration file${declarationFileMap.size > 1 ? 's' : ''}...`);
...
async function bundless(opts) {
    utils_1.logger.info(`Bundless for ${utils_1.chalk.yellow(opts.configProvider.input)} directory to ${utils_1.chalk.yellow(opts.configProvider.configs[0].format)} format`);
    ...
    if (!opts.watch) {
        ...
        utils_1.logger.event(`Transformed successfully in ${Date.now() - startTime} ms (${count} files)`);
    }
    else {
        // watching for watch mode
        utils_1.logger.event(`Start watching ${opts.configProvider.input} directory...`);
        const watcher = utils_1.chokidar
            ...
            .on('change', utils_1.lodash.debounce((filePath) => {
            transformFiles([filePath], opts);
        }, constants_1.WATCH_DEBOUNCE_STEP, { leading: true, trailing: false }))
            ...
            if (relatedMainFile) {
                ...
                utils_1.logger.event(`Bundless ${utils_1.chalk.gray(path_1.default.relative(opts.cwd, relatedMainFile))} is removed`);
            }
        })
            ...
            if (fs_1.default.existsSync(dirDistAbsPath)) {
